  ...
  template: `
    <div class="relative" [class.z-50]="open()">
      ...
      @if (open()) { <div role="listbox" [attr.aria-label]="ariaLabel()" class="absolute top-full left-0 z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-[#315b7e] bg-[#123451] p-1 shadow-[0_14px_32px_rgba(0,0,0,0.35)]">
        @for (option of options(); track option.value) { <button type="button" role="option" [attr.aria-selected]="value() === option.value" (click)="choose(option.value)" class="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left text-sm text-white hover:bg-[#1c486a] focus-visible:bg-[#1c486a]" [class.bg-[#1c486a]]="value() === option.value">
          ...
          @if (value() === option.value) { <ion-icon name="checkmark" class="text-cyan-300" aria-hidden="true"></ion-icon> }
        </button> }
      </div> }
    </div>
  `,
...
export class AppSelect implements ControlValueAccessor {
  ...
  selected(): SelectOption | undefined { return this.options().find((option) => option.value === this.value()); }
