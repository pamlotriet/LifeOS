...
@Component({
  ...
  template: `
    <ion-header app-page-header title="Fuel">
      <nav aria-label="Fuel sections" class="relative z-10 mx-4 mt-2 mb-4 grid grid-cols-3 rounded-full border border-[#5a89b5] bg-gradient-to-b from-[#153a5d] to-[#102b49] p-1 shadow-[0_0_14px_rgba(25,184,237,0.16),inset_0_1px_0_rgba(255,255,255,0.08)]">
          @for (option of segments; track option.value) {
            <a
              [routerLink]="option.route"
              [replaceUrl]="true"
              [attr.aria-current]="selectedSegment() === option.value ? 'page' : null"
              class="flex min-h-11 items-center justify-center gap-1 rounded-full px-1 text-center text-[10px] font-semibold leading-tight no-underline transition-colors"
              [class]="selectedSegment() === option.value
            ...
            >
              @if (option.value === 'consumption') {
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 shrink-0" aria-hidden="true">
                  <path d="M4 21V5a2 2 0 0 1 2-2h7a2 2 0 0 1 2 2v16M3 21h13M7 7h5v4H7zM15 9l3 3v6a2 2 0 0 0 4 0v-6l-2-2" />
                </svg>
              } @else {
                <ion-icon [name]="option.icon" class="shrink-0 text-base" aria-hidden="true"></ion-icon>
              }
