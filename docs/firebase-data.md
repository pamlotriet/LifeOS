...
Books live at `users/{uid}/books/{bookId}` and user-defined mood and genre tags at `users/{uid}/bookTags/{tagId}`. Each book stores its main category, reading details, optional series name and number, tag IDs, and an array of individually identified copies with format and label. The app searches Open Library by title and author, saves a matching Open Library Covers API URL in `coverUrl`, and shows a fallback cover when there is no reliable match. Users do not upload book covers. Deleting a tag removes its ID from affected books in one Firestore commit. The Books rules were deployed to project `fuel-consumption-92c8c`. Three sample books and seven tags were added to the existing user account; all three sample covers were matched and saved from Open Library. Firebase Storage rules no longer permit book-cover uploads.
